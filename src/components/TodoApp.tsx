...
export default function TodoApp({
  ...
  useEffect(() => { fetchTodos(); }, [fetchTodos, scenario]);
    ...
    <div style={{ maxWidth: 540, margin: "0 auto", padding: "2rem 1rem" }}>
      ...
      {error && (
        <div style={{ background: "var(--color-background-danger)", border: "0.5px solid var(--color-border-danger)", borderRadius: "var(--border-radius-md)", padding: "12px 16px", marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 12, color: "var(--color-text-danger)", margin: "4px 0 0", fontFamily: "var(--font-mono)", opacity: 0.8 }}>{error}</p>
          <button onClick={fetchTodos} style={{ marginTop: 8, fontSize: 12 }}>Retry</button>
        </div>
      )}
        ...
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <div style={{ height: 4, width: 120, background: "var(--color-border-tertiary)", borderRadius: 2, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(completedCount / todos.length) * 100}%`, background: "var(--color-text-info)", borderRadius: 2, transition: "width 0.3s ease" }} />
            </div>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
            {todos.map((todo) => (
              <div key={todo.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", transition: "all 0.15s ease" }}>
                <div
                  onClick={() => toggleTodo(todo)}
                  style={{ width: 18, height: 18, borderRadius: "50%", border: todo.completed ? "none" : "1.5px solid var(--color-border-secondary)", background: todo.completed ? "#1D9E75" : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.15s ease" }}
                >
                ...
                <span style={{ flex: 1, fontSize: 14, color: todo.completed ? "var(--color-text-tertiary)" : "var(--color-text-primary)", textDecoration: todo.completed ? "line-through" : "none", transition: "all 0.15s ease" }}>{todo.title}</span>
                <button onClick={() => deleteTodo(todo.id)} style={{ border: "none", background: "transparent", color: "var(--color-text-tertiary)", cursor: "pointer", padding: "2px 6px", fontSize: 16, lineHeight: 1, opacity: 0.4 }} title="Delete">×</button>
              </div>
            ))}
          </div>
        </>
